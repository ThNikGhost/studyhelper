...
- **Дата**: 2026-02-04
- **Сессия**: Реализован парсер расписания ОмГУ (Playwright)

## Общий прогресс
**Фаза**: MVP разработка
**Прогресс**: ~70% (backend готов + парсер, frontend базовый готов)
...
- [x] 85 новых тестов (74 parser + 11 sync)
...
- [ ] Страница расписания (SchedulePage) — план готов
...
### Следующая задача: Тестирование парсера на реальном сайте
1. Установить Playwright браузер: `uv run playwright install chromium`
2. Запустить парсинг: `uv run python -m src.cli.schedule_cli parse --visible --verbose`
3. Адаптировать селекторы под реальную структуру DOM сайта eservice.omsu.ru
...
- **Парсер**: Playwright + SHA-256 хеширование для отслеживания изменений
